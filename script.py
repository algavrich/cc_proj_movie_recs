...
def create_movie_dict() -> defaultdict:
    ...
    for i in range(len(movie_data)):
        ...
        title = unicodedata.normalize('NFKD', movie_data['movie_title'][i]).strip()
...
def get_possible_years(movie_dict: defaultdict, sorted_years: List[str], input_str: str) -> List[str]:
    """Get list of possible release years from user input of partial or complete year string."""
    ...
    if input_len == 4:
        ...
        for i in range(len(sorted_years)):
            if sorted_years[i][:input_len] == input_str:
                curr_idx = i
                while curr_idx < len(sorted_years) and sorted_years[curr_idx][:input_len] == input_str:
...
def recommend(movie_dict: defaultdict) -> None:
    ...
    while rec:
        input_str = input('Type the beginning of the year you would like to see: ')
        if len(input_str) > 4:
            print('Year must be 4 digits or less')
            continue

        else:
            possible_years = get_possible_years(movie_dict, sort_years(movie_dict), input_str)

            if not possible_years:
                print('We don\'t have any recommendations for that year, please try another')
                continue

            elif len(possible_years) == 1:
                if len(input_str) < 4:
                    print(f'There is one year in our system that matches your request: {possible_years[0]}')
                    is_desired = input('Would you like to see movies for this year? [y/n]: ').lower()

                    if is_desired != 'y':
                        continue

                print(f'Here are some movies that were released in {possible_years[0]}')
                display_movies(movie_dict, possible_years[0])

                see_more = input('Would you like to search for a different year? [y/n]: ').lower()
                if see_more != 'y':
                    break

            else:
                print(f'Here are some years that match your request:\n {possible_years}')
